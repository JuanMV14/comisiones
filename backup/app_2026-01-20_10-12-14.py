...
def main():
    ...
    try:
        ...
        with st.sidebar:
            ...
            # Toggle de tema (checkbox Light/Dark)
            ThemeManager.render_theme_switch()
            ...
            try:
                ...
                st.error(f"Error al cargar meta: {str(e)}")
            ...
            with col1:
                if st.button("🔄 Actualizar", use_container_width=True):
                    ...
                    if not st.session_state.get("_updating", False) and not st.session_state.get("_rerunning", False):
            ...
            with col2:
                if st.button("🧹 Limpiar", use_container_width=True):
                    # Evitar múltiples reruns simultáneos
                    if not st.session_state.get("_clearing", False) and not st.session_state.get("_rerunning", False):
        ...
        # Función helper para renderizar tabs con manejo de errores
        def render_tab_safe(tab_index, render_func, *args, **kwargs):
            """Renderiza un tab con manejo de errores"""
            try:
        ...
        with tabs[0]:
            render_tab_safe(0, systems["tab_renderer"].render_executive_dashboard)
        
        # Tab 1: Dashboard General
        with tabs[1]:
            render_tab_safe(1, systems["tab_renderer"].render_dashboard)
        
        # Tab 2: Comisiones
        with tabs[2]:
            render_tab_safe(2, systems["tab_renderer"].render_comisiones)
        
        # Tab 3: Nueva Venta
        with tabs[3]:
            render_tab_safe(3, systems["tab_renderer"].render_nueva_venta)
        
        # Tab 4: Nueva Venta Simple
        with tabs[4]:
            render_tab_safe(4, systems["tab_renderer"].render_nueva_venta_simple)
        
        # Tab 5: Devoluciones
        with tabs[5]:
            render_tab_safe(5, systems["tab_renderer"].render_devoluciones)
        
        # Tab 6: Mensajes para Clientes
        with tabs[6]:
            render_tab_safe(6, systems["tab_renderer"].render_radicacion_facturas)
        
        # Tab 7: Clientes
        with tabs[7]:
            render_tab_safe(7, systems["tab_renderer"].render_clientes)
        
        # Tab 8: Comisiones Mensuales
        with tabs[8]:
            render_tab_safe(8, render_monthly_commissions_tab, systems)
        
        # Tab 9: Catálogo
        with tabs[9]:
            render_tab_safe(9, systems["tab_renderer"].render_catalog_store)
        
        # Tab 10: Análisis de Clientes B2B
        with tabs[10]:
            render_tab_safe(10, systems["client_analysis_ui"].render_gestion_clientes)
        
        # Tab 11: Analytics de Clientes
        with tabs[11]:
            render_tab_safe(11, systems["client_analytics_ui"].render_analytics_dashboard)
...
def load_css():
    ...
    # CSS adicional específico de la app (usa variables CSS del tema)
    st.markdown("""
    <style>
